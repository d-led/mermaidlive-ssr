defmodule MermaidLiveSsr.CountdownFSM do
  ...
  def init(opts) do
    ...
    final_channel = if pubsub_channel == @fsm_updates_channel do
      ...
      IO.puts("FSM init: current_pid=#{inspect(current_pid)}, global_pid=#{inspect(global_pid)}")
      case global_pid do
        ^current_pid ->
          IO.puts("FSM init: Using global channel: #{@fsm_updates_channel}")
          @fsm_updates_channel  # This is the global FSM (pinned match)
        _ ->
          pid_channel = get_channel_for_pid(current_pid)
          IO.puts("FSM init: Using PID-based channel: #{pid_channel}")
          pid_channel  # This is a test FSM, use PID-based channel
      end
    else
      IO.puts("FSM init: Using custom channel: #{pubsub_channel}")
      pubsub_channel  # Use the explicitly provided channel
    end

    IO.puts("FSM init: Final channel: #{final_channel}")
  ...
  defp publish_state_change(new_state, %{pubsub_channel: channel}) do
    IO.puts("FSM publishing state change #{inspect(new_state)} to channel: #{channel}")
